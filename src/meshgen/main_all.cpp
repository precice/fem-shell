...
int main(int argc, char *argv[])
{
    ...
    if (argc != 14)
    {
        std::cout << "usage: ./meshGen type nx ny min_x min_y max_x max_y bcids factor loading ul_lr dead-axis filename\n";
        std::cout << "type: Q|q for Quad-4, T|t for Tri-3\n"
                  << "nx: no. elements on primary axis\n"
                  << "ny: no. elements on secondary axis\n"
                  << "min_x: minimum position on the primary axis\n"
    ...
    {
        std::string str = bcids.substr(0, first);
        std::cout << "blubb: " << str.c_str() << std::endl; // todo: weg!
        t_bcid = std::stoi(str);
    }
    curPos = first+1;
    first = bcids.find_first_of(",", first+1);
    ...
    if (first != std::string::npos)
    {
        std::string str = bcids.substr(curPos, first-curPos); // todo: weg!
        std::cout << "blubb: " << str.c_str() << std::endl;
        r_bcid = std::stoi(str);
    }
...
>>>>>>> 54ce1e24f41fa710080fd2b2687a87533169fc86

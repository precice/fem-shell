...
int main(int argc, char *argv[])
{
    if (argc != 12)
    {
        std::cout << "usage: ./meshgen_q nx ny min_x min_y max_x max_y bcid factor uniform lrtba filename\n";
        return -1;
    }
    int nx = atoi(argv[1]);
    int ny = atoi(argv[2]);
    ...
    os << n_quads << "      # n_elem at level 0, [ type (n0 ... nN-1) ]\n";
    for (unsigned int i = 0; i < quads.size(); i++)
    {
        std::vector<int> cur_quad = quads[i];
        os << "5 " << cur_quad[0] << " " << cur_quad[1] << " " << cur_quad[2] << " " << cur_quad[3] << "\n";
    }
